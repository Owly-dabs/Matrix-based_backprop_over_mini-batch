...
def load_data():
    ...
    This is a nice data format, but for use in nerual networks it's helpful to modify the format 
    of the ``training_dat`` a little. That's done in the wrapper function``load_data_wrapper()``,
    see below.
    """
...
def load_data_wrapper():
    """
    Return a tuple containing ``(training_data, calidation_data,test_data)``.
    ...
    ``y`` is a 10-deimensional numpy.ndarray representing the unit vector
    ...
    # Why doesn't the validation and test results need to be one-hot encoded?
    return(training_data, validation_data, test_data)
